  ...
  operationalMetricList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db
        ...
        .orderBy(desc(operationalMetric.id))
  ...
  operationalMetricById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db
        .select()
        .from(operationalMetric)
        .where(eq(operationalMetric.id, input.id));
  ...
  systemHealthById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db
        .select()
        .from(systemHealth)
        .where(eq(systemHealth.id, input.id));
      return result[0] ?? null;
    }),
