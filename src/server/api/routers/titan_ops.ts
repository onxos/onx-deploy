...
import { titanMaintenance, titanMonitoringLog } from "@/server/db/schema";
...
export const titan_opsRouter = createTRPCRouter({
  titanMonitoringLogList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db
        ...
        .orderBy(desc(titanMonitoringLog.id))
  ...
  titanMonitoringLogById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db
        .select()
        .from(titanMonitoringLog)
        .where(eq(titanMonitoringLog.id, input.id));
  ...
  titanMaintenanceList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db
        .select()
        .from(titanMaintenance)
        .where(conditions.length > 0 ? and(...conditions) : undefined)
        .orderBy(desc(titanMaintenance.id))
        .limit(input?.limit ?? 50)
        .offset(input?.offset ?? 0);
    }),
  ...
  titanMaintenanceById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db
        .select()
        .from(titanMaintenance)
        .where(eq(titanMaintenance.id, input.id));
      return result[0] ?? null;
    }),
