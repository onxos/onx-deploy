...
export const security_reviewRouter = createTRPCRouter({
  securityAuditList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (input?.status)
        conditions.push(eq(securityAudit.status, input.status));
      return db
        .select()
        .from(securityAudit)
  ...
  securityAuditById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db
        .select()
        .from(securityAudit)
        .where(eq(securityAudit.id, input.id));
  ...
  vulnerabilityTrackingList: protectedProcedure
    ...
    .query(async ({ input }) => {
      const conditions: SQL[] = [];
      if (input?.status)
        conditions.push(eq(vulnerabilityTracking.status, input.status));
      return db
        .select()
        .from(vulnerabilityTracking)
        .where(conditions.length > 0 ? and(...conditions) : undefined)
        .orderBy(desc(vulnerabilityTracking.id))
        .limit(input?.limit ?? 50)
        .offset(input?.offset ?? 0);
    }),
  ...
  vulnerabilityTrackingById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db
        .select()
        .from(vulnerabilityTracking)
        .where(eq(vulnerabilityTracking.id, input.id));
      return result[0] ?? null;
    }),
