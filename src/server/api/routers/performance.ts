...
export const performanceRouter = createTRPCRouter({
  performanceMetricList: protectedProcedure
    ...
    .query(async ({ input }) => {
      const conditions = [];
      return db.select().from(performanceMetric).where(conditions.length > 0 ? and(...conditions) : undefined).orderBy(desc(performanceMetric.id)).limit(input?.limit ?? 50).offset(input?.offset ?? 0);
    }),
  ...
  performanceMetricById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db.select().from(performanceMetric).where(eq(performanceMetric.id, input.id));
  ...
  performanceMetricCreate: protectedProcedure
    ...
    .mutation(async ({ _ctx, input }) => {
      const result = await db.insert(performanceMetric).values(input).returning();
      return result[0];
    }),
  ...
  performanceMetricDelete: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      await db.delete(performanceMetric).where(eq(performanceMetric.id, input.id));
      return { success: true };
    }),
  ...
  performanceMetricCount: protectedProcedure
    .use(requirePermission("analytics:read"))
    .query(async () => {
      const result = await db.select({ count: sql<number>`count(*)` }).from(performanceMetric);
  ...
  loadTestResultList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db.select().from(loadTestResult).where(conditions.length > 0 ? and(...conditions) : undefined).orderBy(desc(loadTestResult.id)).limit(input?.limit ?? 50).offset(input?.offset ?? 0);
    }),
  ...
  loadTestResultById: protectedProcedure
    ...
    .input(z.object({ id: z.number() }))
    .query(async ({ input }) => {
      const result = await db.select().from(loadTestResult).where(eq(loadTestResult.id, input.id));
      return result[0] ?? null;
    }),
  ...
  loadTestResultCount: protectedProcedure
    .use(requirePermission("analytics:read"))
    .query(async () => {
      const result = await db.select({ count: sql<number>`count(*)` }).from(loadTestResult);
      return result[0]?.count ?? 0;
    }),
