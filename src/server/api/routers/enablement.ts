...
export const enablementRouter = createTRPCRouter({
  trainingMaterialList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db.select().from(trainingMaterial).where(conditions.length > 0 ? and(...conditions) : undefined).orderBy(desc(trainingMaterial.id)).limit(input?.limit ?? 50).offset(input?.offset ?? 0);
    }),
  ...
  trainingMaterialById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db.select().from(trainingMaterial).where(eq(trainingMaterial.id, input.id));
  ...
  trainingMaterialCreate: protectedProcedure
    ...
    .mutation(async ({ _ctx, input }) => {
      const result = await db.insert(trainingMaterial).values(input).returning();
      return result[0];
    }),
  ...
  trainingMaterialDelete: protectedProcedure
    ...
    .input(z.object({ id: z.number() }))
    .mutation(async ({ input }) => {
      await db.delete(trainingMaterial).where(eq(trainingMaterial.id, input.id));
      return { success: true };
    }),
  ...
  trainingMaterialCount: protectedProcedure
    .use(requirePermission("user:updateRole"))
    .query(async () => {
      const result = await db.select({ count: sql<number>`count(*)` }).from(trainingMaterial);
  ...
  onboardingFlowList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db.select().from(onboardingFlow).where(conditions.length > 0 ? and(...conditions) : undefined).orderBy(desc(onboardingFlow.id)).limit(input?.limit ?? 50).offset(input?.offset ?? 0);
    }),
  ...
  onboardingFlowById: protectedProcedure
    ...
    .input(z.object({ id: z.number() }))
    .query(async ({ input }) => {
      const result = await db.select().from(onboardingFlow).where(eq(onboardingFlow.id, input.id));
      return result[0] ?? null;
    }),
  ...
  onboardingFlowCount: protectedProcedure
    .use(requirePermission("user:updateRole"))
    .query(async () => {
      const result = await db.select({ count: sql<number>`count(*)` }).from(onboardingFlow);
      return result[0]?.count ?? 0;
    }),
