...
export const editorialRouter = createTRPCRouter({
  editorialPolicyList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db.select().from(editorialPolicy).where(conditions.length > 0 ? and(...conditions) : undefined).orderBy(desc(editorialPolicy.id)).limit(input?.limit ?? 50).offset(input?.offset ?? 0);
    }),
  ...
  editorialPolicyById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db.select().from(editorialPolicy).where(eq(editorialPolicy.id, input.id));
  ...
  editorialPolicyCount: protectedProcedure
    .use(requirePermission("article:update"))
    .query(async () => {
      const result = await db.select({ count: sql<number>`count(*)` }).from(editorialPolicy);
  ...
  contentReviewList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db.select().from(contentReview).where(conditions.length > 0 ? and(...conditions) : undefined).orderBy(desc(contentReview.id)).limit(input?.limit ?? 50).offset(input?.offset ?? 0);
    }),
  ...
  contentReviewById: protectedProcedure
    ...
    .input(z.object({ id: z.number() }))
    .query(async ({ input }) => {
      const result = await db.select().from(contentReview).where(eq(contentReview.id, input.id));
      return result[0] ?? null;
    }),
  ...
  contentReviewCount: protectedProcedure
    .use(requirePermission("article:update"))
    .query(async () => {
      const result = await db.select({ count: sql<number>`count(*)` }).from(contentReview);
      return result[0]?.count ?? 0;
    }),
  ...
  publicationScheduleList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db.select().from(publicationSchedule).where(conditions.length > 0 ? and(...conditions) : undefined).orderBy(desc(publicationSchedule.id)).limit(input?.limit ?? 50).offset(input?.offset ?? 0);
    }),
  ...
  publicationScheduleById: protectedProcedure
    ...
    .input(z.object({ id: z.number() }))
    .query(async ({ input }) => {
      const result = await db.select().from(publicationSchedule).where(eq(publicationSchedule.id, input.id));
      return result[0] ?? null;
    }),
  ...
  publicationScheduleDelete: protectedProcedure
    ...
    .input(z.object({ id: z.number() }))
    .mutation(async ({ input }) => {
      await db.delete(publicationSchedule).where(eq(publicationSchedule.id, input.id));
      return { success: true };
    }),
  ...
  publicationScheduleCount: protectedProcedure
    .use(requirePermission("article:update"))
    .query(async () => {
      const result = await db.select({ count: sql<number>`count(*)` }).from(publicationSchedule);
      return result[0]?.count ?? 0;
    }),
