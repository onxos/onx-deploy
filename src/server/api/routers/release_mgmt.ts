...
import { deploymentTracking, releaseRecord } from "@/server/db/schema";
...
export const release_mgmtRouter = createTRPCRouter({
  releaseRecordList: protectedProcedure
    ...
    .input(
      z
        .object({
          status: z
            .enum(["planning", "ready", "deployed", "rolled_back"])
            .optional(),
    ...
    .query(async ({ input }) => {
      ...
      if (input?.status)
        conditions.push(eq(releaseRecord.status, input.status));
      return db
        .select()
        .from(releaseRecord)
        .where(conditions.length > 0 ? and(...conditions) : undefined)
        .orderBy(desc(releaseRecord.id))
  ...
  releaseRecordById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db
        .select()
        .from(releaseRecord)
        .where(eq(releaseRecord.id, input.id));
  ...
  releaseRecordUpdate: protectedProcedure
    ...
    .input(
      z.object({
        id: z.number(),
        status: z.enum(["planning", "ready", "deployed", "rolled_back"]),
      }),
    )
  ...
  deploymentTrackingList: protectedProcedure
    ...
    .query(async ({ input }) => {
      const conditions: SQL[] = [];
      if (input?.status)
        conditions.push(eq(deploymentTracking.status, input.status));
      return db
        .select()
        .from(deploymentTracking)
        .where(conditions.length > 0 ? and(...conditions) : undefined)
        .orderBy(desc(deploymentTracking.id))
        .limit(input?.limit ?? 50)
        .offset(input?.offset ?? 0);
    }),
  ...
  deploymentTrackingById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db
        .select()
        .from(deploymentTracking)
        .where(eq(deploymentTracking.id, input.id));
      return result[0] ?? null;
    }),
