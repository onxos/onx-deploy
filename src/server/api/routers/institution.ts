...
export const institutionRouter = createTRPCRouter({
  institutionSettingList: protectedProcedure
    ...
    .query(async ({ input }) => {
      const conditions = [];
      return db.select().from(institutionSetting).where(conditions.length > 0 ? and(...conditions) : undefined).orderBy(desc(institutionSetting.id)).limit(input?.limit ?? 50).offset(input?.offset ?? 0);
    }),
  ...
  institutionSettingById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db.select().from(institutionSetting).where(eq(institutionSetting.id, input.id));
  ...
  institutionSettingCreate: protectedProcedure
    ...
    .mutation(async ({ _ctx, input }) => {
      const result = await db.insert(institutionSetting).values(input).returning();
      return result[0];
    }),
  ...
  institutionSettingDelete: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      await db.delete(institutionSetting).where(eq(institutionSetting.id, input.id));
      return { success: true };
    }),
  ...
  institutionSettingCount: protectedProcedure
    .use(requirePermission("admin:read"))
    .query(async () => {
      const result = await db.select({ count: sql<number>`count(*)` }).from(institutionSetting);
  ...
  memberManagementList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db.select().from(memberManagement).where(conditions.length > 0 ? and(...conditions) : undefined).orderBy(desc(memberManagement.id)).limit(input?.limit ?? 50).offset(input?.offset ?? 0);
    }),
  ...
  memberManagementById: protectedProcedure
    ...
    .input(z.object({ id: z.number() }))
    .query(async ({ input }) => {
      const result = await db.select().from(memberManagement).where(eq(memberManagement.id, input.id));
      return result[0] ?? null;
    }),
  ...
  memberManagementCreate: protectedProcedure
    ...
    .mutation(async ({ _ctx, input }) => {
      const result = await db.insert(memberManagement).values(input).returning();
      return result[0];
    }),
  ...
  memberManagementDelete: protectedProcedure
    ...
    .input(z.object({ id: z.number() }))
    .mutation(async ({ input }) => {
      await db.delete(memberManagement).where(eq(memberManagement.id, input.id));
      return { success: true };
    }),
  ...
  memberManagementCount: protectedProcedure
    .use(requirePermission("admin:read"))
    .query(async () => {
      const result = await db.select({ count: sql<number>`count(*)` }).from(memberManagement);
      return result[0]?.count ?? 0;
    }),
