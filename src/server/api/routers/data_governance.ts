...
export const data_governanceRouter = createTRPCRouter({
  dataGovernanceRuleList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (input?.status)
        conditions.push(eq(dataGovernanceRule.status, input.status));
      return db
        .select()
        .from(dataGovernanceRule)
        .where(conditions.length > 0 ? and(...conditions) : undefined)
        .orderBy(desc(dataGovernanceRule.id))
  ...
  dataGovernanceRuleById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db
        .select()
        .from(dataGovernanceRule)
        .where(eq(dataGovernanceRule.id, input.id));
  ...
  dataQualityCheckList: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db
        .select()
        .from(dataQualityCheck)
        .where(conditions.length > 0 ? and(...conditions) : undefined)
        .orderBy(desc(dataQualityCheck.id))
        .limit(input?.limit ?? 50)
        .offset(input?.offset ?? 0);
    }),
  ...
  dataQualityCheckById: protectedProcedure
    ...
    .query(async ({ input }) => {
      const result = await db
        .select()
        .from(dataQualityCheck)
        .where(eq(dataQualityCheck.id, input.id));
      return result[0] ?? null;
    }),
