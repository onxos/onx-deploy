 ...
 * Branch-level Permission Scoping layout
 *
...
export default async function BranchPermissionsLayout({
  children,
}: {
  ...
  if (!session) {
    redirect("/login?callbackUrl=/branch-permissions");
  }
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <nav className="flex flex-wrap gap-2 text-sm font-medium">
            {[
              ...
              ["/branch-permissions", "Branch Roles"],
            ].map(([href, label]) => (
              <Link
                key={href}
                href={href}
